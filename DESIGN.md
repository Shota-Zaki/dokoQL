...
- 左の広い作業領域: 問題 + DATABASE
  - 問題文をDATABASEの直上へ配置
  - テーブル一覧 / テーブル定義 / 初期データ / 現在データを大きく表示
  - 学習時に問題条件と元データを同じ視線移動方向で確認できる構成にする
- 右の操作領域: SQL EDITOR + RESULT
  - SQL入力欄
  - 実行 / 判定 / 解答例 / ヒント / リセット
  - 実行結果、更新件数、エラー、判定理由

DATABASEはSQL EDITOR / RESULTより広い表示領域を確保し、横幅のあるテーブルでも列を確認しやすくする。
...
- >= 768px: 問題一覧を上部横並び、左側を問題 + DATABASEの広い領域、右側をSQL EDITOR + RESULTとする2カラム型ワークベンチ
- < 768px: 1カラム + タブ

中間幅でもDATABASEをドロワーへ退避せず、テーブル参照を主要作業領域として維持する。横幅が不足する場合はテーブル内部の横スクロールで対応する。
