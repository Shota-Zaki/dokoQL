...
問題確認とテーブル参照を優先し、横長画面では次の配置とする。

- 最上段: 問題ナビゲーション
  - 題材切替
  - Chapter切替
  - 状態絞り込み
  - 問題一覧を横方向へ並べ、必要に応じて横スクロール
  - 回答状態・正誤・見直し状態を各問題に表示
- 左の広い作業領域: 問題 + DATABASE
  - 問題文をDATABASEの直上へ配置
  - テーブル一覧
  - テーブル定義
  - 初期データ / 現在データ
  - 横幅のあるテーブルでも列を確認しやすい表示面積を優先
- 右の操作領域: SQL EDITOR + RESULT
  - SQL入力欄
  - 実行 / 判定 / 解答例 / ヒント / リセット
  - 実行結果
  - 更新件数
...
DATABASEはSQL EDITOR / RESULTより広い表示領域を確保し、「問題文を読む → テーブルを確認する」の視線移動を縦方向へ揃える。
...
- >= 1180px: 上部に横並び問題一覧 + 左の広い問題/DATABASE + 右のSQL EDITOR/RESULT
- 768px–1179px: 同じ2カラム構成を維持し、DATABASEはドロワーへ退避しない
- < 768px: 1カラム + タブ

中間幅ではテーブル内部の横スクロールを許容し、DATABASE自体を隠さない。
...
- パレット見出しに `最小化` / `展開` ボタンを置く。
- 最小化時はSQLパーツ、テーブル、カラム、設定を隠し、`クリック入力` の見出しと `展開` ボタンだけを残す。
- 最小化してもSQLエディタの入力内容やカーソル位置には影響しない。
- 最小化状態はブラウザローカルへ保存し、ページ再読込・問題移動・題材切替後も維持する。
- PC・タブレット・スマートフォンのすべてで同じ操作を提供する。
