...
4. クリック入力の自動調整は `0.68` まで縮小でき、従来より全カテゴリが見切れにくい。
5. ResizeObserver等による自己リサイズの再帰ループがない。
6. 実行 / 判定 / DB初期化 / 次の問題が動作する。
7. `npm run check` が成功する。
8. `npm run build:pages` が毎回クリーンな `_site/` を生成する。
9. GitHub Pagesは `_site/` のみを公開する。