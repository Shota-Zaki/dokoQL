...
4. ResizeObserver等による自己リサイズの再帰ループがない。
5. 実行 / 判定 / DB初期化 / 次の問題が動作する。
6. `npm run check` が成功する。
7. `npm run build:pages` が毎回クリーンな `_site/` を生成する。
8. GitHub Pagesは `_site/` のみを公開する。
